...
class PoolingCQ(nn.Module):
    ...
    def forward(self, x):
        ...
        x = Quantization.apply(x, self.constant)
        return x
...
class ClampModule(nn.Module):
    def __init__(self, min_val, max_val):
        super(ClampModule, self).__init__()
        self.min_val = min_val
        self.max_val = max_val
...
class QuantizationModule(nn.Module):
    def __init__(self, constant=100):
        super(QuantizationModule, self).__init__()
        self.constant = constant

    def forward(self, x):
        return Quantization.apply(x, self.constant)
...
def conv_bn_relu(in_channels, out_channels, kernel_size, stride, padding, groups=1):
    ...
    # step1
    result.add_module('clamp', ClampModule(min_val=0, max_val=Clp_max))
    result.add_module('quantize', QuantizationModule(T))
    return result
...
class RepVGGplusBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # step3
        out = torch.clamp(out, 0, Clp_max)
        out = Quantization_(out,T)
...
class RepVGGplus(nn.Module):
    ...
    def _build_aux_for_stage(self, stage):
        ...
        # step5
        return nn.Sequential(downsample, PoolingCQ(min_val=0, max_val=Clp_max, constant = T), nn.Flatten(), fc)
        # return nn.Sequential(downsample, nn.AdaptiveAvgPool2d(1), ClampModule(min_val=0, max_val=Clp_max), QuantizationModule(constant = T), nn.Flatten(), fc)
